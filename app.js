...
const seedWines = [
  {
    ...
    reviews: [
      { id: "local-r1", personaId: "mina", note: "?????Β?ル윲?? ????鶯ㅺ동??ш낄援???? ??? ?????????????????嚥▲굧??????????袁④뎬?轅붽틓?節됰쑏??鶯??????쎛 ?????룸㈇而??????굿?耀붾굝????? ??????????ル뒌????", createdAt: "2026-04-08" },
      { id: "local-r2", personaId: "soyeon", note: "???????? ??????????? ????????嚥▲굧???????????? ??????筌믨퀣議?", createdAt: "2026-04-09" }
    ]
  },
  {
    id: "w2",
    ...
    reviews: [
      { id: "local-r3", personaId: "jun", note: "???????밸븶??뫢??????μ떜媛?걫?繹먃????轅붽틓??????????癲??됀???耀붾굝????????紐껊섈??????????癰궽븍럵??鶯????爰???", createdAt: "2026-04-07" },
      { id: "local-r4", personaId: "eun", note: "???????????? ????釉랁닑???롪퍓媛??????낅??????癲???????????????????????꾨굴????????轅붽틓??筌뚮랭沅??????????????곗뒩筌????黎앸럽?????", createdAt: "2026-04-10" }
    ]
  }
];
...
function addCustomAroma(category, type, groupName, rawValue) {
  ...
  return { ok: true, message: `'${value}' ???????????꾨굴???????????????곸죩.` };
}
...
function renderAromaSelector(category, type) {
  ...
  selectedHost.innerHTML = values.length
    ...
    : '<span class="muted">?????獄쏅챶留??癰귙룗猷?????壤굿??Β????/span>';
  ...
  selector.innerHTML = `
    <div class="aroma-add-row">
      ...
      <input type="text" data-aroma-input="${category}" placeholder="????癲ル슢??????????耀붾굝?????????????꾨굴??">
      <button type="button" class="ghost-button" data-aroma-add="${category}" style="color:var(--text); border-color:var(--line); background:var(--surface-strong)">??????꾨굴??</button>
    </div>
    <div class="aroma-add-status" data-aroma-status="${category}">?????????????????μ떜媛?걫?롪퍊?붺댚????????????????????????????????壤굿??Β????轅붽틓??????</div>
...
async function handleReviewDelete(wineId, reviewId) {
  ...
  const ok = window.confirm(`'${wine.name}' ?????얠뺏??븍툙????????????影??젘???`);
  if (!ok) {
    return;
  }
...
async function handlePersonaDelete() {
  ...
  const ok = window.confirm(`${persona.name} persona??????????影??젘??? ?????怨멸텛????????얠뺏??븍툙???????????????ル꺄???????ㅼ뒭????????轅붽틓??????`);
  if (!ok) {
    return;
  }
...
async function handleImageLookup() {
  ...
  if (!query.primary) {
    el.fetchStatus.textContent = "????饔낅떽??????됰씧媛뺡툣?????Producer??????遺븍き??몄Ŀ?? ???????쇨덫櫻????欲꼲????饔낅떽??????";
    return;
  }
  ...
  el.fetchStatus.textContent = "??????⑤９??????????鶯ㅺ동??????????곌떽釉붾????????????????遺븍き??몄Ŀ?? ?耀붾굝???????? ???????ㅻ쑄?癲ル슢캉??뮻??????곌떽釉붾??? ?????醫딇떍???????????????耀붾굝????? ?????????????????饔낅떽????????????嚥▲굧?????????????????곸죩.";
  ...
  try {
    ...
    if (!state.imageCandidates.length) {
      setUploadFallbackVisible(true, "??????⑤９??????????鶯ㅺ동????? ?????곌떽釉붾??? ???????????耀붾굝????? fallback????濡?씀?濾?? ?耀붾굝?????????붾눀?????饔낅떽??????????????????獄쏅챶留?????耀붾굝??????? ?耀붾굝????鶯????獒???????? ??????耀붾굝??????????????關?쒎첎??????");
      el.fetchStatus.textContent = data.note || "?????耀붾굝????? ?????獄쏅챶留?????耀붾굝??????? ?耀붾굝????鶯????獒???????? ?????獄쏅챶留?????????fallback?????????ㅻ쑄?????欲꼲????饔낅떽??????";
      return;
    }
    ...
    if (!el.wineImage.value.trim() && state.imageCandidates[0]?.image_url) {
      ...
      el.fetchStatus.textContent = data.note || `${state.imageCandidates.length}?????ル뒌?????????耀붾굝????? ?????獄쏅챶留?????耀붾굝???????????????? ??????⑤９????????곌떽釉붾?????????????????? ??饔낅떽????鶯ㅺ동???????????????곸죩.`;
    }
  } catch (error) {
    setUploadFallbackVisible(true, "???????????????????쇨덫??????ル뒌?? ???轅붽틓??影?놁쟼?鶯????????fallback???????????? ?????????곕츥????????믩베?????????몃뱥????????????????????耀붾굝????????????????????????????????곸죩.");
    el.fetchStatus.textContent = `?????耀붾굝????? ?????獄쏅챶留???????⑥ル????????????ㅼ뒩?? ${error.message || "Cloudflare Function ?????API ????μ떜媛?걫??????饔낅떽????????????欲꼲????饔낅떽??????"}`;
  }
}
async function handlePriceLookup() {
  const query = buildWineLookupQuery();
  if (!query.primary) {
    el.fetchStatus.textContent = "????饔낅떽??????됰씧媛뺡툣????????汝뷴젆?琉????????μ떜媛?걫??곸돸壤??? ????遺븍き??몄Ŀ?? ???????쇨덫櫻????欲꼲????饔낅떽??????";
    return;
  }
  ...
  if (existingWine?.averagePrice && !el.winePrice.value.trim()) {
    ...
    el.fetchStatus.textContent = `?????????????롮쾸?椰???⑤챶猷??????'${existingWine.name}'???????ル뒌?????耀붾굝????????????쇰┛?癲?????遺븍き??몄Ŀ?? ?????????Β?ル윲???????`;
    return;
  }

  el.fetchStatus.textContent = "?????ル뒌??????饔낅떽?????????????⑥ル???????..";
  try {
    const data = await requestWineLookup("price", query);
    if (data.average_price) {
      el.winePrice.value = data.average_price;
      el.fetchStatus.textContent = data.note || "?????ル뒌??????饔낅떽??????????????밸븶筌믩끃????????????????곸죩.";
      return;
    }
    el.fetchStatus.textContent = "??????????????쎛???耀붾굝??????? ?耀붾굝????鶯????獒???????? Wine-Searcher API ????????黎앸럽????븍쇊?용∥?????????쇨덫櫻????饔낅떽????????????欲꼲????饔낅떽??????";
  } catch (error) {
    el.fetchStatus.textContent = `?????ル뒌????????⑥ル????????????ㅼ뒩?? ${error.message || "Cloudflare Function ?????API ????μ떜媛?걫??????饔낅떽????????????欲꼲????饔낅떽??????"}`;
  }
}
