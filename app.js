...
const AROMA_OPTIONS = {
  ...
  secondary: {
    ...
    white: {
      ...
      "Sweet Oak": ["Caramel", "Butterscotch", "Toffee", "Cr?????Brulee"],
...
const seedWines = [
  {
    ...
    reviews: [
      { id: "local-r1", personaId: "mina", note: "???怨쀫뎐?? ?嶺??影袁る젺??? ??? ?????????ル뭽?????롪퍓肉???????댄뱼癲ル슣鍮??彛????쎛 ????룸객??춯?온?轅붽틓??? ????????ル봿????", createdAt: "2026-04-08" },
      { id: "local-r2", personaId: "soyeon", note: "???????? ??????????? ????????롪퍓肉????潁???傭? ??????뽰젵.", createdAt: "2026-04-09" }
    ]
  },
  {
    id: "w2",
    ...
    reviews: [
      { id: "local-r3", personaId: "jun", note: "????ш끽維쀩?????濚밸Ŧ?김???癲ル슢?????????泳?뿀???轅붽틓?????⑸떭??????????볥군??彛??됯샴???", createdAt: "2026-04-07" },
      { id: "local-r4", personaId: "eun", note: "???????????? ???遺븍き?寃밸윿???덊뒌?????渦????????????????????살퓢癲??????癲ル슢?뤺キ????????????釉뚯뺏????棺堉?댆??", createdAt: "2026-04-10" }
    ]
  }
];
...
function addCustomAroma(category, type, groupName, rawValue) {
  ...
  return { ok: true, message: `'${value}' ?????????살퓢?????????????낆젵.` };
}
...
function populatePersonaOptions() {
  const reviewOptions = state.personas.length
    ...
    : '<option value="">Persona???雅?퍔瑗ⓩ뤃?? ??μ떜媛?걫??곸돥???癲ル슢?ｅ젆???/option>';
...
function updateWineNameMeta() {
  const name = el.wineName.value.trim();
  if (!name) {
    el.wineNameMeta.textContent = "???????????μ떜媛?걫??곸돥???????꿔꺂????酉⑷텣?????숈?????⑥ル섣?????댁삩?????꿔꺂???????? ???숆강?붺춯????? ???癲???????????怨쀫뎐????????";
    return;
  }

  const wine = state.wines.find((item) => item.name.toLowerCase() === name.toLowerCase());
  if (!wine) {
    el.wineNameMeta.textContent = "??????꿔꺂????癒?떻??戮?츐??????⑤챷竊??癲ル슢????? ????????????????袁⑸즴????癲ル슢?????";
    return;
  }

  el.wineNameMeta.textContent = `${wine.reviews.length}?????숆강?붺춯??嶺????쎛 ??μ떜媛?걫??곸돥???????????????꿔꺂????釉뚮뜦??????낆젵.${wine.producer ? ` Producer: ${wine.producer}.` : ""}`;
}
...
function renderPersonas() {
  ...
  el.personaGrid.innerHTML = personas.length
    ...
    : '<div class="empty-state">?????밸븶??볧돯???μ떜媛?걫??곸돥???persona???ル봿?? ?????욱룏???????낆젵. ????댁삩????숆강??????壤굿?????????????轅붽틓?????key in ????觀????꿔꺂?????</div>';
  attachTasteTabs();
...
function renderAromaSelector(category, type) {
  ...
  selectedHost.innerHTML = values.length
    ...
    : '<span class="muted">?????밸븶??볧돯?????節떷????/span>';
  ...
  selector.innerHTML = `
    <div class="aroma-add-row">
      ...
      <input type="text" data-aroma-input="${category}" placeholder="????筌뤾쑵????????轅붽틓?????????살퓢??">
      <button type="button" class="ghost-button" data-aroma-add="${category}" style="color:var(--text); border-color:var(--line); background:var(--surface-strong)">????살퓢??</button>
    </div>
    <div class="aroma-add-status" data-aroma-status="${category}">????????????????濚밸Ŧ寃㎩쳞???????????????????癲????????????節떷???癲ル슢?????</div>
...
async function handleLogin(event) {
  ...
  if (!email || !password) {
    el.authStatus.textContent = "?????????됰씭???????嶺?????亦껋꼦裕㎩쳞??????⑤챷竊????觀????꿔꺂?????";
    return;
  }

  el.authStatus.textContent = "?黎??筌?????..";
  const { error } = await state.supabase.auth.signInWithPassword({ email, password });
  el.authStatus.textContent = error ? `?黎??筌????????怨뚯댅: ${error.message}` : "?黎??筌???꿔꺂???????μ떜媛?슙?癰귥쥙????????????낆젵.";
}
...
async function handleSignup() {
  ...
  const password = el.adminPassword.value.trim();
  if (!email || !password) {
    el.authStatus.textContent = "???ル봿???????ㅿ폍筌??????????됰씭???????嶺?????亦껋꼦裕㎩쳞??雅?퍔瑗ⓩ뤃?? ?????⑤챷竊????觀????꿔꺂?????";
    return;
  }

  el.authStatus.textContent = "????댁삩????숆강??????壤굿????????袁⑸즴?????..";
  const { error } = await state.supabase.auth.signUp({ email, password });
  el.authStatus.textContent = error
    ? `???ル봿?????????怨뚯댅: ${error.message}`
    : "???ル봿????????거?????????밸븶???????????낆젵. ????????꿔꺂????癒κ섶?????꾩룆梨?????????낆젵???轅붽틓???????雅?퍔瑗ⓩ뤃?? ??꿔꺂??틝????????觀????꿔꺂?????";
}
...
async function handleReviewSave(event) {
  ...
  if (!payload.personaId || !payload.name || !payload.summary || !payload.overallScore) {
    if (!payload.personaId) {
      alert("???숆강?붺춯?????鶯ㅺ동???????????밸븶??persona???雅?퍔瑗ⓩ뤃?? ??μ떜媛?걫??곸돥?????觀????꿔꺂?????");
    } else if (!payload.overallScore) {
      alert("Overall score???????⑤챷竊????觀????꿔꺂?????");
    }
    return;
  }
  ...
  if (Number.isNaN(numericScore) || numericScore < 0 || numericScore > 100) {
    alert("Overall score??0~100 ????釉뚰??援??????⑤챷竊????觀????꿔꺂?????");
    return;
  }
...
async function handlePersonaDelete() {
  ...
  const ok = window.confirm(`${persona.name} persona??????????ｋ젒??? ????댁삩??????숆강?붺춯?????鶯ㅺ동???醫듽렒 ???怨뚰뇠???떐??癲ル슢?????`);
  if (!ok) {
    return;
  }
...
async function handleImageLookup() {
  ...
  if (!query.primary) {
    el.fetchStatus.textContent = "????꿔꺂????븍갭夷?????Producer???雅?퍔瑗ⓩ뤃?? ?????⑤챷竊????觀????꿔꺂?????";
    return;
  }
  ...
  el.fetchStatus.textContent = "?????곷쿀?????????影????????蹂κ텤?熬곎逾???????雅?퍔瑗ⓩ뤃?? ?轅붽틓?????? ?????쇨덧?筌먦렜逾?????蹂κ텤??? ????얠뺏癲??????????????轅붽틓??? ????????癲?嶺??????꿔꺂??틝????????롪퍓肉?????????????낆젵.";
  ...
  try {
    ...
    if (!state.imageCandidates.length) {
      setUploadFallbackVisible(true, "?????곷쿀?????????影???? ????蹂κ텤??? ???????????轅붽틓??? fallback?關?쒎첎?嫄?? ?轅붽틓??熬곥끇釉?????꿔꺂??틝??????????????밸븶?????轅붽틓????? ?轅붽틓??彛?臾믪뮏?鶯?????? ??????轅붽틓???????????嚥싲갭횧????");
      el.fetchStatus.textContent = data.note || "?????轅붽틓??? ?????밸븶?????轅붽틓????? ?轅붽틓??彛?臾믪뮏?鶯?????? ?????밸븶??????野??fallback???????쇨덧?????觀????꿔꺂?????";
      return;
    }
    ...
    if (!el.wineImage.value.trim() && state.imageCandidates[0]?.image_url) {
      ...
      el.fetchStatus.textContent = data.note || `${state.imageCandidates.length}???ル봿?????????轅붽틓??? ?????밸븶?????轅붽틓?????????????? ?????곷쿀??????蹂κ텤?????????????????? ??꿔꺂???影????????????낆젵.`;
    }
  } catch (error) {
    setUploadFallbackVisible(true, "???癲?????????????⑤챷逾???ル봿?? ??癲ル슢?ｅ젆?彛?????野??fallback???????????? ???????ㅼ뒧???????裕뼘?????ㅳ늾????????????????????轅붽틓??????傭?끆???????????????????낆젵.");
    el.fetchStatus.textContent = `?????轅붽틓??? ?????밸븶??????곗뒭?????????怨뚯댅: ${error.message || "Cloudflare Function ?????API ???濚밸Ŧ??????꿔꺂??틝????????觀????꿔꺂?????"}`;
  }
}
async function handlePriceLookup() {
  const query = buildWineLookupQuery();
  if (!query.primary) {
    el.fetchStatus.textContent = "????꿔꺂????븍갭夷????????β뼯援???????濚밸Ŧ援욑쭕??? ?雅?퍔瑗ⓩ뤃?? ?????⑤챷竊????觀????꿔꺂?????";
    return;
  }
  ...
  if (existingWine?.averagePrice && !el.winePrice.value.trim()) {
    ...
    el.fetchStatus.textContent = `??????????μ떜媛?걫??곸돥??????'${existingWine.name}'?????ル봿?????轅붽틓???????ш끽踰椰??雅?퍔瑗ⓩ뤃?? ???????怨쀫뎐???????`;
    return;
  }

  el.fetchStatus.textContent = "???ル봿??????꿔꺂???????????곗뒭??????..";
  try {
    const data = await requestWineLookup("price", query);
    if (data.average_price) {
      el.winePrice.value = data.average_price;
      el.fetchStatus.textContent = data.note || "???ル봿??????꿔꺂??????????ш끽維뽳쭩?????????????낆젵.";
      return;
    }
    el.fetchStatus.textContent = "????????????쎛???轅붽틓????? ?轅붽틓??彛?臾믪뮏?鶯?????? Wine-Searcher API ????????棺堉?댆猿껎맪??????⑤챷竊????꿔꺂??틝????????觀????꿔꺂?????";
  } catch (error) {
    el.fetchStatus.textContent = `???ル봿???????곗뒭?????????怨뚯댅: ${error.message || "Cloudflare Function ?????API ???濚밸Ŧ??????꿔꺂??틝????????觀????꿔꺂?????"}`;
  }
}
  ...
  el.imageCandidateGrid.innerHTML = state.imageCandidates.length
    ? state.imageCandidates.map((candidate, index) => `
      <div class="candidate-card">
        <img src="${escapeHtml(candidate.image_url)}" alt="?????????轅붽틓??? ?????밸븶???${index + 1}">
        <div class="candidate-meta">
          <strong>${escapeHtml(candidate.image_source || "Image candidate")}</strong>
          <span>${escapeHtml(candidate.matched_query || "")}</span>
        </div>
        <button class="ghost-button" type="button" data-image-candidate="${index}" style="color:var(--text); border-color:var(--line); background:var(--surface-strong)">???????轅붽틓??? ????/button>
      </div>
    `).join("")
    : '<div class="empty-state">?????곷쿀?????????影???? ????蹂κ텤??? ???????????轅붽틓??? ???????癲ル슢??????????밸븶?????轅붽틓?????????????????낆젵. ?????밸븶??볧돯????怨쀫뎐?????????밸븶???援?醫묒춸????쎛 ?????욱룏???????낆젵.</div>';
...
function applyImageCandidate(candidate) {
  if (!candidate?.image_url || !isUsableImageUrl(candidate.image_url)) {
    el.fetchStatus.textContent = "????節떷????????밸븶???援?醫묒춸????쎛 ???濚밸Ŧ?김???????轅붽틓??? URL???????밸븶????嚥싲갭횧???춹??????⑤뜪???? ?????源낅돹??????";
    return;
  }
  el.wineImage.value = candidate.image_url;
  syncImagePreview(candidate.image_source || "");
  setUploadFallbackVisible(false);
  el.fetchStatus.textContent = `?????轅붽틓??? ?????밸븶?????????⑤뜪????????????낆젵${candidate.image_source ? ` (${candidate.image_source})` : ""}.`;
}
...
function setUploadFallbackVisible(visible, note = "") {
  ...
  if (el.uploadFallbackNote) {
    el.uploadFallbackNote.textContent = note || "?????곷쿀?????????影???? ????蹂κ텤??? ???????????轅붽틓??? fallback?關?쒎첎?嫄?? ???????????轅붽틓????????????????轅붽틓?????????野??? ?????";
  }
}
...
async function handleImageUpload(event, sourceLabel) {
  ...
  try {
    ...
    setUploadFallbackVisible(true, "???癲????????????????轅붽틓????? ??β뼯援????癲?????袁ｋ쨨嶸↓?fallback ?????轅붽틓??????ル봿?? ?????⑤뜪???????????낆젵. ????얠뺏癲???????μ떝?롳쭗?????癲ル슢?????");
    el.fetchStatus.textContent = `${sourceLabel} ?????轅붽틓??????????⑤뜪????????????낆젵.`;
  } catch (_error) {
    el.fetchStatus.textContent = `${sourceLabel} ?轅붽틓??影?뽧걤?????????⑤챷逾???ル봿?? ??ш끽維뽳쭩?좊쐪筌먲퐢?????????????낆젵. ?????遊붋??????轅붽틓??????????ㅻ쿋??????節떷?????觀????꿔꺂?????`;
  } finally {
    event.target.value = "";
  }
}
...
function syncImagePreview(sourceLabel = "") {
  ...
  if (!imageUrl || !isUsableImageUrl(imageUrl)) {
    ...
    el.imagePreviewCaption.textContent = imageUrl
      ? "????節떷??????ル봿???????濚밸Ŧ?김???????轅붽틓??????ル봿?? ?????밸븶???????遺븍き?寃밸윿???????繹먮굟爰????癲?嶺??? ?????源낅돹?????? ?????밸븶???????節떷???????????野??? ?????쇨덧?????觀????꿔꺂?????"
      : "???癲?????????轅붽틓????? ?????밸븶?????轅붽틓?????????野?????????????轅붽틓????????????꿔꺂??틝???????????????????낆젵.";
    return;
  }
