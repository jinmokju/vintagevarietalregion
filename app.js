...
function describeTasteVector(taste, mode) {
  const acidity = taste.acidity <= 2 ? "?? ??? ???? ?? ??" : taste.acidity >= 6 ? "??? ?? ??? ???" : "?? ???? ????";
  const body = taste.body <= 2 ? "??? ?? ??? ???" : taste.body >= 6 ? "??? ???" : "?? ???";
  const oak = taste.oak <= 2 ? "? ?? ??? ?? ?? ????" : taste.oak >= 6 ? "?? ??? ????" : "?? ???? ??? ??? ??";
  ...
  const profile = mode === "white"
    ? (taste.fruitProfile <= 2 ? "?? ?? ??" : taste.fruitProfile >= 6 ? "???? ??" : "?? ??? ????? ????")
    : (taste.fruitProfile <= 2 ? "?? ??? ??" : taste.fruitProfile >= 6 ? "?? ??? ??" : "??? ?? ???? ????");
  return `${acidity}, ${body}, ${oak}, ${drive} ${profile} ???`;
}
...
function buildPersonaHeadline(red, white) {
  const cues = [];
  cues.push(red.acidity <= 3 || white.acidity <= 3 ? "??? ???? ?? ?? ??" : "??? ??? ???? ?? ?? ??");
  cues.push(red.oak >= 6 && white.oak >= 6 ? "??? ??? ??? ?? ??? ?" : red.oak <= 3 || white.oak <= 3 ? "??? ????? ??? ????? ?" : "??? ??? ?? ??? ???? ???? ?");
  cues.push(red.body <= 3 || white.body <= 3 ? "?? ??? ???? ??? ?? ?? ?? ??" : red.body >= 6 && white.body >= 6 ? "??? ??? ??? ???? ??" : "???? ???? ??? ?? ??");
