...
function buildPersonaHeadline(red, white) {
  const cues = [];
  cues.push(red.acidity <= 3 || white.acidity <= 3 ? "산도와 리프트를 먼저 보는 취향" : "과실과 질감의 밸런스를 먼저 보는 취향");
  cues.push(red.oak >= 6 && white.oak >= 6 ? "오크를 절제한 클래식 결에 가까운 편" : red.oak <= 3 || white.oak <= 3 ? "오크의 존재감까지 기꺼이 받아들이는 편" : "오크는 과하지 않게 조율된 스타일을 선호하는 편");
  cues.push(red.body <= 3 || white.body <= 3 ? "풍미 밀도와 존재감이 또렵한 병에 손이 가는 타입" : red.body >= 6 && white.body >= 6 ? "린하고 투명한 질감을 선호하는 타입" : "무게보다 완성도와 균형을 보는 타입");
...
function buildModeSummary(taste, mode, favorites) {
  ...
  const acidityLine = taste.acidity <= 3 ? "산도가 살아 있어 와인을 위로 끌어올리는 구조" : taste.acidity >= 6 ? "산도가 낮아 보다 둥글고 편안한 구조" : "산도 균형이 안정적으로 잡힌 구조";
  const bodyLine = taste.body <= 3 ? "볼륨감과 농밀함이 분명한 바디" : taste.body >= 6 ? "린하고 날렵한 바디" : "중간 밀도로 정리된 바디";
  const oakLine = taste.oak <= 3 ? "뉴 오크의 향미를 기꺼이 받아들이는 편" : taste.oak >= 6 ? "오크 개입은 최대한 절제된 쪽을 선호" : "오크는 존재하되 과하지 않은 밸런스를 선호";
  const favoriteLine = favorites.length ? `${favorites.join(", ")} 계열에 특히 반응하고, ` : "";
  return `${favoriteLine}${fruitSide}, ${acidityLine}, ${bodyLine}, ${oakLine}.`;
}

function describeFruitPreference(taste, mode) {
  if (mode === "white") {
    const drive = taste.fruitDriven <= 3 ? "과실이 또렵한 화이트를 선호하며" : taste.fruitDriven >= 6 ? "미네랄과 리니어한 결이 선명한 화이트를 선호하며" : "과실과 미네랄의 균형이 좋은 화이트를 선호하며";
    const profile = taste.fruitProfile <= 2 ? "열대 과실 뉘앙스에 더 끌립니다" : taste.fruitProfile >= 6 ? "시트러스 결이 선명한 병에 더 끌립니다" : "열대 과실과 시트러스가 교차하는 결을 편안하게 봅니다";
    return `${drive} ${profile}`;
  }
  const drive = taste.fruitDriven <= 3 ? "과실이 전면에 드러나는 레드를 선호하며" : taste.fruitDriven >= 6 ? "세이보리와 어시 톤이 살아 있는 레드를 선호하며" : "과실과 세이보리의 밸런스가 좋은 레드를 선호하며";
  const profile = taste.fruitProfile <= 2 ? "다크 프루트 결에 더 반응합니다" : taste.fruitProfile >= 6 ? "레드 프루트 결에 더 반응합니다" : "레드와 다크 프루트가 교차하는 결을 자연스럽게 받아들입니다";
  return `${drive} ${profile}`;
}
